...
export const initializeApp = () => ( dispatch ) => { // thunk делает ассинхронную операцию которая раньше делалсь в компоненте, в єтой санке делаем что то для инициализации.Эту санку будем диспатчить в Арр
    let promise = dispatch( getAuthUserData() ); // задисипатчим получение инициализационных/аутаризационных данных. диспатч вернет нам то, что мы ретурнем из ф=ции, которая в него заключена (?), в даном случае из санки
    // dispatch(smthelse())
    // dispatch(smthelse())
