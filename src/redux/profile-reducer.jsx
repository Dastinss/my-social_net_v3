...
export const getUserProfile = ( userId ) => async ( dispatch ) => { // создаем санку - ф-цию, которая принимаем ф-цию диспатч и делает внутри какието ассинхронные операции и различные мелкие астины
    // userAPI.getProfile( userId ) // #90 закоментили потому что ввели await
    let response = await userAPI.getProfile( userId ) // #90 ввели await
        // .then( response => { // #90 закоментили потому что ввели await //делаем запрос на сервер с гет запросом для которого достаточно урл адреса, и говорим "когда сервак даст ответ, затем выполни этот колл бек/эту ф-цию" в которую в качестве ответа от сервера придет респонс
            dispatch( setUserProfile( response.data ) );
        // } );
}

export const getStatus = ( userId ) => async ( dispatch ) => { // # 73 создаем санку - ф-цию, которая принимаем ф-цию диспатч и делает внутри какието ассинхронные операции и различные мелкие астины
    let response = await profileAPI.getStatus( userId ) // #90 закоментили потому что ввели await
        // .then( response => { // #90 закоментили потому что ввели await //делаем запрос на сервер с гет запросом для которого достаточно урл адреса, и говорим "когда сервак даст ответ, затем выполни этот колл бек/эту ф-цию" в которую в качестве ответа от сервера придет респонс
            dispatch( setStatus( response.data ) ); // когда плучим статус, мы его засетаем
        // } );
}

export const updateStatus = ( status ) => async ( dispatch ) => { // # 73 создаем санку - ф-цию, которая принимаем ф-цию диспатч и делает внутри какието ассинхронные операции и различные мелкие астины
    let response = await profileAPI.updateStatus( status ) // #90 закоментили потому что ввели await
        // .then( response => { // #90 закоментили потому что ввели await //делаем запрос на сервер с гет запросом для которого достаточно урл адреса, и говорим "когда сервак даст ответ, затем выполни этот колл бек/эту ф-цию" в которую в качестве ответа от сервера придет респонс
