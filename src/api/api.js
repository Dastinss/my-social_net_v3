...
export const userAPI = { // создали обьект который содержит методы, это упаковка для всех методов для группировки
    ...
    follow( userId ) { // перенес в уроке 66 с Users и убрал все ненужное, что есть в instance вверху
        return instance.post( `https://social-network.samuraijs.com/api/1.0/follow/${userId}`) //вторым параметром почетому передаем пустой обьект - раскажут позднее
    },
    unfollow( userId ) { // перенес в уроке 66 с Users и убрал все ненужное, что есть в instance вверху
        return instance.delete( `https://social-network.samuraijs.com/api/1.0/unfollow/${userId}`)  //НЕ принимает второй параметр - раскажут позднее. Вторым параметром идет параметр настройки
    }
}
