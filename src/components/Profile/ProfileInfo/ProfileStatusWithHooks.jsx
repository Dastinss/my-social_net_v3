...
const ProfileStatusWithHooks = (props) => { //84 это функцилнальная компонента, которая создали из такой же классовой ProfileStatus.jsx
    ...
    return (
            <div>
                { !editMode &&
                    <div>
                        <span onDoubleClick={activateEditMode}>{props.status || '========= '}</span> {/* поставили БЕЗ bind , т.к. стоит стрелочная ф-ция fn=()=>*. status показіваем ИЗ пропсов, т.к. сервер не обновился */}
                    </div>
                }
                { editMode &&
                    <div>
                        <input onChange={onStatusChange} autoFocus={true} onBlur={deactivateEditMode} value={status}/> {/* поставили С bind , т.к. стоит определяющая  ф-ция (fn()  ), status показіваем не из пропсов, а из локального стейта */}
                    </div>
                }
            </div>
        )
