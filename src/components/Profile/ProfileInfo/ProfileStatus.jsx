        ...
        return (
            <div>
                {!this.state.editMode && //если у нас editMode НЕ тру, то мы отобразим ОДНУ дивку
                    <div>
                        <span onDoubleClick={ this.activateEditMode }>{this.props.status || '========='}</span> {/* поставили БЕЗ bind , т.к. стоит стрелочная ф-ция fn=()=>*. status показіваем ИЗ пропсов, т.к. сервер не обновился */}
                    </div>
                }
                {this.state.editMode && //если у нас editMode тру, то мы отобразим ДРУГУЮ дивку
                    <div>
                        <input onChange={this.onStatusChange} autoFocus={true} onBlur={ this.deactivateEditMode } value={this.state.status}/> {/* поставили С bind , т.к. стоит определяющая  ф-ция (fn()  ), status показіваем не из пропсов, а из локального стейта */}
                    </div>
                }
            </div>
        )
